...
- **P-6: Map view.** Plot published entries on an interactive map using `entries.lat`/`entries.lon` (or nearest photo/calendar event location). `lat`/`lon` columns already exist on `Diary` and `Photo` after 2026-05-28 commits. Needs lat/lon propagation to `entries` table (or derive on load from linked photos). Frontend: Leaflet or Mapbox GL JS map component at `/diaries/[id]/map`. Likely files: Alembic migration (entries lat/lon), new Next.js route. **Unblocked by Phase 2 item 24** (per-entry location & time — adds `entries.lat/lon` + `tz_override` + Open-Meteo city-search UI to the entry form).
