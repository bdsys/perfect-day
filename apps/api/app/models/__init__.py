...
class User(TimestampMixin, SoftDeleteMixin, Base):
    ...
    email: Mapped[str] = mapped_column(String(320), unique=True, nullable=False)  # citext via migration
    ...
    # relationships
    social_identities: Mapped[list["SocialIdentity"]] = relationship(back_populates="user", cascade="all, delete-orphan")
    oauth_tokens: Mapped[list["OAuthToken"]] = relationship(back_populates="user", cascade="all, delete-orphan")
    diaries: Mapped[list["Diary"]] = relationship(back_populates="owner", foreign_keys="Diary.owner_user_id", cascade="all, delete-orphan")
    refresh_tokens: Mapped[list["RefreshToken"]] = relationship(back_populates="user", cascade="all, delete-orphan")
    notification_preferences: Mapped["NotificationPreferences | None"] = relationship(back_populates="user", cascade="all, delete-orphan")
...
class SocialIdentity(TimestampMixin, Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("provider IN ('google','facebook','apple')", name="ck_social_identities_provider"),
    )
...
class Diary(TimestampMixin, SoftDeleteMixin, Base):
    ...
    cover_photo_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("photos.id", ondelete="SET NULL"))
    notifications_muted: Mapped[bool] = mapped_column(Boolean, nullable=False, server_default="false")
    photos_backfill_days_max: Mapped[int] = mapped_column(Integer, nullable=False, server_default="90")
    hard_delete_after: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    ...
    permissions: Mapped[list["DiaryPermission"]] = relationship(back_populates="diary", cascade="all, delete-orphan")
    invitations: Mapped[list["Invitation"]] = relationship(back_populates="diary", cascade="all, delete-orphan")
    scan_job: Mapped["ScanJob | None"] = relationship(back_populates="diary", cascade="all, delete-orphan", uselist=False)
    scan_runs: Mapped[list["ScanRun"]] = relationship(back_populates="diary", cascade="all, delete-orphan")
    calendar_filters: Mapped[list["DiaryCalendarFilter"]] = relationship(back_populates="diary", cascade="all, delete-orphan")
    backfill_runs: Mapped[list["BackfillRun"]] = relationship(back_populates="diary", cascade="all, delete-orphan")
...
class DiaryPermission(TimestampMixin, Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    diary_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("diaries.id", ondelete="CASCADE"), nullable=False)
    user_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
...
class Invitation(TimestampMixin, Base):
    ...
    accepted_by_user_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"))

    diary: Mapped["Diary"] = relationship(back_populates="invitations")

    __table_args__ = (
        CheckConstraint("role IN ('viewer','editor')", name="ck_invitations_role"),
    )
...
class Entry(TimestampMixin, SoftDeleteMixin, Base):
    ...
    parent_entry_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("entries.id", ondelete="SET NULL"))
    ...
    entry_photos: Mapped[list["EntryPhoto"]] = relationship(back_populates="entry", cascade="all, delete-orphan")
    enrichments: Mapped[list["Enrichment"]] = relationship(back_populates="entry", cascade="all, delete-orphan")
    llm_generations: Mapped[list["LLMGeneration"]] = relationship(back_populates="entry", cascade="all, delete-orphan")
    edit_diffs: Mapped[list["EntryEditDiff"]] = relationship(back_populates="entry", cascade="all, delete-orphan")
...
class DiaryPhoto(Base):
    ...
    diary_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("diaries.id", ondelete="CASCADE"), primary_key=True)
    photo_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("photos.id", ondelete="CASCADE"), primary_key=True)


class EntryPhoto(Base):
    __tablename__ = "entry_photos"

    entry_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("entries.id", ondelete="CASCADE"), primary_key=True)
    photo_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("photos.id", ondelete="CASCADE"), primary_key=True)
...
class Enrichment(TimestampMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint("entry_id", "kind", name="uq_enrichments_entry_kind"),
    )
...
class EntryEditDiff(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    entry_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("entries.id", ondelete="CASCADE"), nullable=False)
    llm_generation_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("llm_generations.id", ondelete="CASCADE"), nullable=False)
...
class ScanJob(TimestampMixin, Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    diary_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("diaries.id", ondelete="CASCADE"), unique=True, nullable=False)
...
class ScanRun(TimestampMixin, Base):
    ...
    __table_args__ = (
        CheckConstraint("triggered_by IN ('beat','manual','backfill','admin')", name="ck_scan_runs_triggered_by"),
        CheckConstraint("status IN ('running','success','partial','failed')", name="ck_scan_runs_status"),
    )
...
class DiaryCalendarFilter(TimestampMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint("diary_id", "google_calendar_id", name="uq_diary_calendar_filters_diary_gcal"),
    )
...
class NotificationPreferences(TimestampMixin, Base):
    ...
    expo_push_tokens: Mapped[list[str]] = mapped_column(ARRAY(Text), nullable=False, server_default="{}")
    ...
    kinds_disabled: Mapped[list[str]] = mapped_column(ARRAY(Text), nullable=False, server_default="{}")
...
class Notification(Base):
    ...
    channel_push_status: Mapped[str] = mapped_column(String(10), nullable=False, server_default="pending")
    channel_email_status: Mapped[str] = mapped_column(String(10), nullable=False, server_default="pending")
    channel_inapp_status: Mapped[str] = mapped_column(String(10), nullable=False, server_default="pending")
...
class MagicLinkToken(Base):
    ...
    __table_args__ = (
        Index("ix_magic_link_tokens_email_expires", "email", "expires_at"),
    )
