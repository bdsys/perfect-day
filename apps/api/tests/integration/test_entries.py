...
class TestListEntries:
    async def test_list_returns_entries(self, client):
        ...
        auth = {"Authorization": f"Bearer {token}"}
        await client.post(f"/v1/diaries/{diary['id']}/entries",
                           json={"entry_date": "2025-06-01"}, headers=auth)
        await client.post(f"/v1/diaries/{diary['id']}/entries",
                           json={"entry_date": "2025-06-02"}, headers=auth)
    ...
    async def test_status_filter(self, client):
        ...
        auth = {"Authorization": f"Bearer {token}"}
        entry = (await client.post(f"/v1/diaries/{diary['id']}/entries",
                                    json={"entry_date": "2025-06-01"}, headers=auth)).json()
        ...
        published = await client.get(f"/v1/diaries/{diary['id']}/entries?status=published", headers=auth)
...
class TestViewerAccessControl:
    async def test_viewer_cannot_see_drafts(self, client):
        ...
        viewer_r = await client.post("/v1/auth/register",
