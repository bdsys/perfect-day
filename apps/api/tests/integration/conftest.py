...
async def client(db_engine, redis_container, monkeypatch) -> AsyncGenerator[AsyncClient, None]:
    ...
    monkeypatch.setenv("DATABASE_URL", str(db_engine.url).replace("postgresql+asyncpg", "postgresql+asyncpg"))
    monkeypatch.setenv("DATABASE_URL_SYNC", str(db_engine.url).replace("+asyncpg", ""))
    ...
    db_module._session_factory = async_sessionmaker(db_engine, expire_on_commit=False, class_=AsyncSession)
